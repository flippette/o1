...
class Simulator:
  ...
  /** Advances the most recently launched simulation by one step, moving all dissatisfied
    ...
    * @see [[startNew]] */
  def moveResidents() =
    Random.shuffle(this.dissatisfiedResidents).foreach(resident =>
      Random.shuffle(this.findDemographic(Vacant)).headOption
        .foreach(vacant => this.cityMap.swap(resident, vacant))
    )
  ...
  def findDemographic(demographic: Demographic): Vector[GridPos] =
    this.allAddresses.filter(this.cityMap(_) == demographic)
  ...
  def dissatisfiedResidents: Vector[GridPos] =
    this.allAddresses
      .filter(this.cityMap(_) != Vacant)
      .filter(pos =>
        val demo = this.cityMap(pos)
        val neighbors = this.cityMap.neighbors(pos, true).filter(_ != Vacant)
        neighbors.count(_ == demo).toDouble / neighbors.length * 100 < this.similarityDesired
      )
