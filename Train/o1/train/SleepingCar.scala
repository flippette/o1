...
class SleepingCar(val numberOfCabins: Int) extends TrainCar:
  ...
  def numberOfFreePlaces =
    val range = 1 to numberOfCabins
    val frees = range.map(this.numberOfFreeBedsInCabin)
    frees.sum
  ...
  def reservePlaces(numberOfPeople: Int) =
    val cabins = numberOfPeople / BedsPerCabin + (if numberOfPeople % BedsPerCabin != 0 then 1 else 0) // temporary
    if this.emptyCabinCount < cabins then
      false
    else
      for cabinNumber <- 1 to cabins do // stepper
        this.reserveCabin(this.findEmptyCabin)
      true

  /** Returns the number of empty cabins in the car. */
  def emptyCabinCount =
    (1 to this.numberOfCabins).count(this.isEmptyCabin)
  ...
  def reserveCabin(cabinNumber: Int): Boolean =
    if !this.isEmptyCabin(cabinNumber) then
      false
    else
  ...
  private def findEmptyCabin: Int =
    (1 to this.numberOfCabins).find(this.isEmptyCabin).getOrElse(-1)
