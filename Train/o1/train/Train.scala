...
import o1.train.Train.MaximumLength
...
class Train(val description: String):
  ...
  def addCar(car: TrainCar) =
    if this.numberOfCars < MaximumLength then
  ...
  def car(numberOfCar: Int) =
    if numberOfCar > this.numberOfCars then None
    else Some(this.cars(numberOfCar - 1))
...
object Train:
  ...
  /** The maximum length, in cars, of a train. */
