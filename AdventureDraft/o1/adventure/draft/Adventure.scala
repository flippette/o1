...
class Adventure:
  ...
  val middle      = Area("Forest", "You are somewhere in the forest. There are a lot of trees here.\nBirds are singing.")
  val northForest = Area("Forest", "You are somewhere in the forest. A tangle of bushes blocks further passage north.\nBirds are singing.")
  val southForest = Area("Forest", "The forest just goes on and on.")
  val clearing    = Area("Forest Clearing", "You are at a small clearing in the middle of forest.\nNearly invisible, twisted paths lead in many directions.")
  val tangle      = Area("Tangle of Bushes", "You are in a dense tangle of bushes. It's hard to see exactly where you're going.")
  val home        = Area("Home", "Home sweet home!")
  ...
  /** Prints out a message that is to be displayed to the player at the end of the game.
    * The message will be different depending on whether the player has completed the quest. */
  ...
  def playTurn(): Unit =
    ...
    if action.execute(this.player) then
      this.turnCount += 1
    else
