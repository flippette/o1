...
class FamilyMember(val name: String, val children: Vector[FamilyMember]):
  ...
  def numberOfDescendingGenerations: Int =
    if this.isChildless then 0
    else 1 + this.children.map(_.numberOfDescendingGenerations).max
  ...
  def numberOfDescendants: Int =
    if this.isChildless then 0
    else this.children.length + this.children.map(_.numberOfDescendants).sum
  ...
  def mostChildren: FamilyMember =
    this.everyoneBelow.maxByOption(_.numberOfChildren) match
      case Some(child) if child.numberOfChildren > this.numberOfChildren => child
      case _ => this
  ...
  def everyoneBelow: Vector[FamilyMember] =
    this.children.flatMap(_.everyoneBelow).prepended(this)

  /** Returns a string representation of the person. */
  override def toString =
    def childString = this.children.mkString(",").take(500)

    if this.isChildless then this.name else s"$name($childString)"
