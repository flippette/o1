...
trait RobotBrain(initialName: String, val body: RobotBody):
  ...
  def robotInFront = this.world(this.locationInFront).robot.flatMap(_.brain)
  ...
  def isStuck = !(this.mayMove(CompassDir.North)
    || this.mayMove(CompassDir.West)
    || this.mayMove(CompassDir.South)
    || this.mayMove(grid.CompassDir.East))
  ...
  def advanceCarefully() =
    val move = this.squareInFront.isEmpty
    if move then this.body.moveTowards(this.body.facing)
    move
