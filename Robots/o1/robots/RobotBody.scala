...
class RobotBody(val world: RobotWorld, initialLocation: GridPos, initialFacing: CompassDir):
  ...
  def neighboringSquare(direction: CompassDir) = this.world(this.coordinates.neighbor(direction))

  /** Turns the robot 90 degrees clockwise. */
  def spinClockwise() = this.spinTowards(this.facing.clockwise)
  ...
  def takeTurn() = if this.isIntact then this.brain.foreach(_.controlTurn())
