...
class RobotWorld(floorWidth: Int, floorHeight: Int) extends Grid[Square](floorWidth + 2, floorHeight + 2):
  ...
  private def initialSquare(x: Int, y: Int) = (x + 1, y + 1) match {
    case (1 | this.width, _) | (_, 1 | this.height) => Wall
    case _ => Floor()
  }
  ...
  def addRobot(initialLocation: GridPos, initialFacing: CompassDir) =
    this.robots :+= RobotBody(this, initialLocation, initialFacing)
    this (initialLocation).addRobot(this.robots.last)
    this.robots.last
  ...
  def advanceTurn() =
    this.robotWithNextTurn.map(_.takeTurn()).foreach(_ => this.turn = (this.turn + 1) % this.numberOfRobots)
  ...
  def robotWithNextTurn: Option[RobotBody] = this.robots.lift(this.turn)
  ...
  /** Causes all the robots in the world to take a turn, starting with the one whose turn it is next.
    ...
    * @see [[advanceTurn]]
    * @see [[RobotBody.takeTurn]] */
  def advanceFullRound() = this.robots
    .drop(this.turn)
    .concat(this.robots.take(this.turn))
    .foreach(_.takeTurn())
end RobotWorld
