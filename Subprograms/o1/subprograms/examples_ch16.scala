...
def playDoodads(player: String): Unit =
  val InitialDoodads = 12
  o1.gui.O1SwingDefaults()
  import o1.gui.Dialog.*

  def isValidPick(pick: Int): Boolean =
    pick == 1 || pick == 2

  trait Turn(val doodadsLeft: Int):
    def gameContinues = this.doodadsLeft > 0
    def isHumanWin: Boolean
    def play(): Turn

  class HumanTurn(doodadsLeft: Int) extends Turn(doodadsLeft):
    def isHumanWin = false
    def play() =
      val pickUnlessCanceled = requestInt(s"There are $doodadsLeft Doodads remaining. How many will you pick up?", isValidPick, "Please take either 1 or 2.", Centered)
      pickUnlessCanceled.map(pick => ComputerTurn(this.doodadsLeft - pick) ).getOrElse(Cancelled)

  class ComputerTurn(doodadsLeft: Int) extends Turn(doodadsLeft):
    def isHumanWin = !this.gameContinues
    def play() =
      val computerPick = this.bestPick(this.doodadsLeft)
      display(s"I'll take $computerPick Doodad${(if computerPick == 1 then "" else "s")}.", Centered)
      HumanTurn(this.doodadsLeft - computerPick)
    private def bestPick(doodadsLeft: Int): Int =
      val attempted = doodadsLeft % 3
      if isValidPick(attempted) then attempted else 2

  object Cancelled extends Turn(0):
    def isHumanWin = false
    def play() = Cancelled

  display(s"Welcome to the game of DOODADS, $player!\nIn the beginning, there are $InitialDoodads Doodads.\n" +
          "We will take turns choosing to pick up either 1 or 2 Doodads.\nThe one who picks up the last Doodad wins.", Centered)
  val firstTurn: Turn = HumanTurn(InitialDoodads)
  val endState = LazyList.iterate(firstTurn)( _.play() ).dropWhile( _.gameContinues ).head
  if endState.isHumanWin then
    display(s"You won, $player! But I think you’ve been up to something sneaky to achieve that...", Centered)
  else
